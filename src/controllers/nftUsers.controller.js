...
// const { handlePostSignup } = require('../utils/handlePostSignupProcess');
// const { handleBrandApproval } = require('../utils/handleBrandApproval');
...
const getIndividualBrand = catchAsync(async (req, res) => {
  const { id } = await pick(req.params, ['id']);
  const findResult = await nftUserService.getIndividualBrand(id);
  if (findResult) {
    sendResponse(res, httpStatus.OK, findResult, null);
  } else {
    sendResponse(res, httpStatus.BAD_REQUEST, null, 'Please provide valid Id to approve user');
  }
});
